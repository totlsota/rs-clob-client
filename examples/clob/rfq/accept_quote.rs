//! Demonstrates accepting an RFQ quote (requester-side execution).
//!
//! This endpoint requires a fully-specified, signed order payload (EIP-712), so this example is
//! intentionally "low-level". If you are coming from the TS client, note that the TS helper
//! constructs this payload for you.
//!
//! Run with tracing enabled:
//! ```sh
//! RUST_LOG=info,hyper_util=off,hyper=off,reqwest=off,h2=off,rustls=off cargo run --example rfq_accept_quote --features clob,rfq,tracing
//! ```
//!
...
use polymarket_client_sdk::clob::types::request::AcceptRfqQuoteRequest;
...
async fn main() -> anyhow::Result<()> {
    ...
    match client.accept_quote(&request).await {
        Ok(_resp) => info!(endpoint = "accept_quote", status = "OK"),
        Err(e) => error!(endpoint = "accept_quote", error = %e),
    }
